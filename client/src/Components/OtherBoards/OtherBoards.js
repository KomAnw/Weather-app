...
export default function OtherBoards(props) {
  const listItem = props.data.consolidated_weather.slice(1).map((item, ind) => {
    return (
      <div key={item.id} className="card">
        <div className="card__main-info">
          ...
          <p className="card__date">
            {item.applicable_date.split("-").reverse().join("-")}
          </p>
        </div>
        <div className="card__other-info">
          <p className="card__item">
            Minimum temperature&nbsp;
            <span className="card__itwm-bold">
          ...
          <p className="card__item">
            Wind speed&nbsp;
            <span className="card__itwm-bold">
              {Math.round(item.wind_direction)}&#176;
            </span>
          </p>
          <p className="card__item">
            Wind direction&nbsp;
  ...
  return <div className="OtherBoards">{listItem}</div>;
}
